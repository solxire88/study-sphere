...
export default function About({ totalStudents, schedule }) {
  ...
  const metrics = [
    { Icon: ContactRound, label: "Number of Students", value: totalStudents },
    { Icon: Star, label: "Average Rating", value: `${averageRating} / 5.0` },
    { Icon: CalendarCheck, label: "Class Schedule", value: schedule },
  ];
          ...
          <table className="text-white border-collapse w-full lg:w-3/4">
            ...
            <tbody>
              {metrics.map(({ Icon, label, value }, index) => (
                <tr key={label} className="hover:bg-[#0A0F24] transition-all duration-300 animate-fade-in">
                  <td className="py-3 px-4 text-center whitespace-nowrap">
                    <div className="flex flex-col items-center justify-center space-y-1">
                      <Icon className="w-6 h-6 mb-1 transition-transform duration-300 hover:-translate-y-1 hover:text-[#1E90FF] animate-float-icon" />
                  ...
                  <td className={`py-3 px-4 text-center animate-fade-in delay-${(index + 1) * 100} whitespace-nowrap`}>
                    <span className="inline-block text-base hover:translate-x-2 hover:scale-105 transition-transform duration-300 hover:text-[#1E90FF]">
                      {value}
                    </span>
                  </td>
